...
97,d&255,d>>8&255,d>>16&255,d>>24&255]);b=0;for(d=44;b<v;++b){var g=l[b];g=-32767>g?-32767:32767<g?32767:g;e[d++]=g&255;e[d++]=g>>8&255}return e};this.getData=function(b,d){for(var e=2*Math.floor(44100*b),g=Array(d),f=0;f<2*d;f+=1){var k=e+f;g[f]=0<b&&k<l.length?l[k]/32768:0}return g}};qa.s();var ra=setInterval(function(){if(da=1<=qa.o())clearInterval(ra),c.onmousedown()},0);navigator.mediaDevices.getUserMedia({audio:!0}).then(function(d){ea=d;c.onmousedown();N=new MediaRecorder(d);N.ondataavailable=function(d){R[A].b.src=URL.createObjectURL(d.data);n=M.currentTime-L;R[A].b.currentTime=.1+n+R[A].of;R[A].b.play();A=0}});c.onmousedown=function(d){if(M)(d=Math.ceil(((d.touches?d.touches[0].pageY:d.pageY)-c.offsetTop-64)/oa))&&x?z||(A=A!=d?d:0):u?(u=0,ca=!ca):x?u=1:(L=M.currentTime,Aa());else if(ea&&da){M=new (window.AudioContext||window.webkitAudioContext);I=M.createAnalyser();I.connect(M.destination);H=M.createGain();H.connect(I);for(d=0;6>d;++d)if(R[d].c=M.createAnalyser(),R[d].c.fftSize=2*T.length,5>d&&R[d].c.connect(H),5==d){var t=M.createMediaStreamSource(ea);t.connect(R[d].c)}else d&&(t=M.createMediaElementSource(R[d].b),
t.connect(R[d].c));d=qa.l();M.decodeAudioData(d.buffer,function(d){R[0].g=d;L=M.currentTime;Aa();Sa(0)})}};function Aa(){u=0;x=1;for(var d=0;5>d;++d)if(R[d].g){var t=M.createBufferSource();t.buffer=R[d].g;t.connect(R[d].c);t.start(0);t.onended=function(){u?x=u=0:(L=M.currentTime,N&&A&&(z?(N.stop(),z=0,H.gain.setValueAtTime(1,M.currentTime),n=M.currentTime-L,R[A].of+=n):(H.gain.setValueAtTime(.1,M.currentTime),N.start(),z=1,R[A].of=n=M.currentTime-L)),Aa())}}else R[d].b.src&&(R[d].b.currentTime=M.currentTime-L+.1,R[d].b.play());n=0}
